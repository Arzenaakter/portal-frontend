...
export default function AdminVideosPage() {
  ...
  const filtered = videos
    ...
    .sort((a, b) => {
      ...
      if (sort.field === "views") return ((a.views || 0) - (b.views || 0)) * dir;
      if (sort.field === "createdAt") return (new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()) * dir;
  ...
  const SortIcon = ({ field }: { field: SortField }) => (
    <span className="ml-1 inline-flex flex-col" style={{ opacity: sort.field === field ? 1 : 0.3 }}>
      <ChevronUp size={10} style={{ color: sort.field === field && sort.dir === "asc" ? "var(--primary)" : "var(--muted-foreground)", marginBottom: -2 }} />
      <ChevronDown size={10} style={{ color: sort.field === field && sort.dir === "desc" ? "var(--primary)" : "var(--muted-foreground)" }} />
    </span>
  );
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Main content */}
      <div className={`flex flex-col flex-1 overflow-hidden transition-all duration-300 ${isFormOpen ? "hidden lg:flex lg:w-1/2 xl:w-3/5" : "w-full"}`}>
        <AdminHeader title="Videos" subtitle={`${videos.length} total videos`} />
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="flex items-center justify-between gap-4 mb-5 flex-wrap">
            ...
            <button
              onClick={() => { setFormMode("add"); setEditingVideo(null); }}
              ...
              style={{ background: "var(--primary)", color: "black", fontFamily: "var(--font-display)" }}>
              <Plus size={16} />
              Add Video
            </button>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="border-b" style={{ borderColor: "var(--border)", background: "rgba(30,32,38,0.5)" }}>
                    <th className="text-left px-5 py-3.5 w-12">
                      <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted-foreground)" }}>#</span>
                    </th>
                    {[
                    ...
                    <th className="text-right px-5 py-3.5">
                      <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted-foreground)" }}>Actions</span>
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center py-16 text-sm" style={{ color: "var(--muted-foreground)" }}>
                        No videos found.
                      </td>
                    </tr>
                    ...
                    <tr key={video.id}
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-semibold leading-tight line-clamp-1 mb-0.5"
                      ...
                      <td className="px-5 py-4">
                        <div className="flex flex-col gap-1">
                          <span className="text-xs font-medium px-2 py-0.5 rounded-lg w-fit"
                            style={{ background: "rgba(232,255,71,0.08)", color: "var(--primary)" }}>
                            {video.category}
                          </span>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => { setEditingVideo(video); setFormMode("edit"); }}
                            className="p-2 rounded-lg transition-all hover:opacity-80"
                            style={{ border: "1px solid var(--border)", color: "#47c8e8" }}
                            title="Edit">
                            <Pencil size={13} />
                          </button>
                          <button
                            ...
                            className="p-2 rounded-lg transition-all hover:opacity-80"
                            style={{ border: "1px solid rgba(255,68,68,0.3)", color: "var(--destructive)" }}
                            title="Delete">
                            <Trash2 size={13} />
                          </button>
                        </div>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                    <button key={p} onClick={() => setPage(p)}
                      ...
                      style={{
                        ...
                        color: p === page ? "black" : "var(--muted-foreground)",
      ...
      {/* Side panel form */}
      {isFormOpen && (
        <div className="flex flex-col w-full lg:w-1/2 xl:w-2/5 border-l overflow-hidden"
          style={{ borderColor: "var(--border)", background: "var(--background)" }}>
          <VideoForm
            mode={formMode!}
            initialData={editingVideo || undefined}
            onSubmit={formMode === "add" ? handleAdd : handleEdit}
            onCancel={() => { setFormMode(null); setEditingVideo(null); }}
          />
        </div>
      )}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="rounded-2xl p-6 w-full max-w-sm"
            ...
            <h3 className="text-center font-bold text-lg mb-2" style={{ fontFamily: "var(--font-display)" }}>
              Delete Video?
            </h3>
            <p className="text-center text-sm mb-1" style={{ color: "var(--muted-foreground)" }}>
              This will permanently delete
            </p>
            <p className="text-center text-sm font-semibold mb-6" style={{ color: "var(--foreground)" }}>
